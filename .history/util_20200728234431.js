...
function valida_cnpj(cnpj) {
    ...
    var width = cnpj.length - 2
    var numeros = cnpj.substring(0, width);
    var digitos = cnpj.substring(width);
    var soma = 0;
    var pos = width - 7;

    for (var i = width; i >= 1; i--) {
        soma += numeros.charAt(width - i) * pos--;
        if (pos < 2)
            pos = 9;
    }
    ...
    width = width + 1;
    numeros = cnpj.substring(0, width);
    soma = 0;
    pos = width - 7;
