...
cnpj_cpf.addEventListener('change', () => {
    ...
    // if(verificarCNPJ_CPF(cnpj_cpf.value) == false) {
	...
	// var form = $(this).closest("form");
			...
			if (verificarCNPJ_CPF(this.value) === false) {
				showErrorMsg(form, "danger", "Registro Inválido.");
				// $("#cnpj_cpf, .cnpj_cpf_mask").val("").focus();
			} else {
...
//     if(value.length == 11) return valida_cpf(value);
//     if(value.length == 14) return valida_cnpj(value);
